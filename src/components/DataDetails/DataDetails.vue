...
export default {
  ...
  setup(props, ctx) {
    ...
    // ---- Watchers ----
    watchEffect(async () => {
      if (datum.value && datum.value.created) {
        details.value = datum.value
        return
      }

      if (datum.value && datum.value._id && datum.value._modelType) {
        const { data } = await rest.get(
          `${datum.value._modelType}/${datum.value._id}`
        )
        details.value = data
        return
      }
    ...
    watchEffect(async () => {
      if (datum.value && datum.value._modelType === 'item') {
        const { data } = await rest.get(
          `item/${datum.value._id}/files`
        )
